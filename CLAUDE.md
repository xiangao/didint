...
- `R/did_int_2x2.R` — Xu (2023) 2×2 case. Thin wrapper around
  `.dr_atte()`.
- `R/did_int_dynamic.R` — Xu (2026) §I event study under common
  adoption.
- `R/did_int_staggered.R` — Xu (2026) §II staggered adoption. **Crucial
  subtlety**: the cross-cell aggregation uses joint-IF stacking
  (per-cell IFs aligned to unit IDs and summed across cells), not
  independent-cells SEs. Cells share the never-treated comparison, so
  independent-cells SEs underestimate by ~30% (verified in
  `inst/sims/`).
...
averages the DR signal over the **full sample** (S_M), not the `{G = g}`
stratum. This is the paper’s definition. There’s a regression test in
`tests/testthat/test-2x2-zdep-estimand.R` that pins this — don’t drop
it. (Original bug had the average over `{G = g}`; only caught when
treatment effect varied with z.)
...
- **Docs**: pkgdown auto-builds and deploys to `gh-pages` on push to
  master. Live at <https://xiangao.github.io/didint/>. Roxygen man pages
  are committed (regenerate with `roxygen2::roxygenise()`).
...
4.  Update `inst/sims/findings.md` if numerical results materially
    shift.
